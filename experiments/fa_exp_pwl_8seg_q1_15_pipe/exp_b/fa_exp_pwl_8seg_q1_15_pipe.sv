module fa_exp_pwl_8seg_q1_15_pipe (
  ...
  logic               s2_sat_zero;
  logic [15:0]        s2_y0;
  logic [15:0]        s2_interp_hi;
  logic [15:0]        s2_interp_lo;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      s2_valid     <= s1_valid;
      s2_sat_zero  <= s1_sat_zero;
      s2_y0        <= s1_y0;
      s2_interp_hi <= delta_hi * s1_frac;
      s2_interp_lo <= (delta_lo * s1_frac) >> 8;
    end
  end

  always_comb begin
    interp_q1_15 = s2_interp_hi + s2_interp_lo;
    if (s2_sat_zero) begin
      exp_q1_15 = 16'd0;
    end else if (s2_y0 > interp_q1_15) begin
      exp_q1_15 = s2_y0 - interp_q1_15;
    end else begin
      exp_q1_15 = 16'd0;
    end
  end
