module fa_o_normalize_block_pipe #(
  ...
  logic signed [95:0]  s0_mul [LANES];
    ...
    if (!rst_n) begin
      ...
      if (i_valid) begin
        for (int lane = 0; lane < LANES; lane++) begin
          ...
          s0_mul[lane] <= lane64(i_acc_flat, lane) * $signed({1'b0, i_recip_q16_16});
        end
      end
      ...
      if (s0_valid) begin
        for (int lane = 0; lane < LANES; lane++) begin
          ...
          end else if (s0_mul[lane] >= 0) begin
            s1_res[lane] <= (s0_mul[lane] + 96'sd2147483648) >>> 32;
          end else begin
            s1_res[lane] <= (s0_mul[lane] - 96'sd2147483648) >>> 32;
          end
        end
