...
async def test_fp32_mul_q16_matches_cmodel(dut):
    ...
    if should_dump_rows():
        csv_path = write_case_csv("fa_fp32_mul_q16", seed, rows)
        dut._log.info("fp32_mul_q16: detail_csv=%s", csv_path)
    dut._log.info("fp32_mul_q16: seed=%d samples=%d mismatches=%d MAE=%.6f MaxAE=%.6f", seed, samples, mismatches, mae, maxe)
    assert mismatches == 0
