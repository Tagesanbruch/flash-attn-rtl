module fa_online_softmax_ctx (
  ...
  logic [15:0]        s1_exp_old, s1_exp_new;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    ...
    if (!rst_n) begin
      ...
      if (s0_valid) begin
        ...
        s1_value <= s0_value;
        s1_exp_old <= exp2_approx(s0_m_prev - ((s0_score > s0_m_prev) ? s0_score : s0_m_prev));
        s1_exp_new <= exp2_approx(s0_score - ((s0_score > s0_m_prev) ? s0_score : s0_m_prev));
      end else begin
        s1_valid <= 1'b0;
      end
      ...
      if (s1_valid) begin
        idx = s1_ctx_id;
        l_scaled_q32_31 = s1_l_prev * s1_exp_old;
        l_new_q16_16 = l_scaled_q32_31[46:15] + {15'd0, s1_exp_new, 1'b0};
        acc_scaled_q33_31 = s1_acc_prev * $signed({1'b0, s1_exp_old});
        v_mul_q9_23 = $signed({1'b0, s1_exp_new}) * s1_value;
        acc_new_q16_16 = acc_scaled_q33_31[46:15] + {{5{v_mul_q9_23[33]}}, v_mul_q9_23[33:7]};

        m_state[idx] <= s1_m_new;
        ...
        s2_ctx_id <= s1_ctx_id;
        s2_m_new <= s1_m_new;
        s2_l_new <= l_new_q16_16;
        s2_acc_new <= acc_new_q16_16;
      end else begin
        s2_valid <= 1'b0;
      end

      o_valid <= s2_valid;
      o_row_done <= s2_valid && s2_row_end;
      o_ctx_id <= s2_ctx_id;
      o_m_q8_8 <= s2_m_new;
      o_l_q16_16 <= s2_l_new;
      o_acc_q16_16 <= s2_acc_new;
    end
  end
endmodule