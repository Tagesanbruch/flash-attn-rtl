...
long long count_matmul = 0;
long long count_softmax = 0;
long long count_rmsnorm = 0;
long long count_attn_dot = 0;
long long count_attn_v = 0;

// --- Trace Globals ---
FILE *trace_fp = NULL;
int do_trace = 0; // True when we want to trace the current pos
// -----------------------
...
float *forward(Transformer *transformer, int token, int pos, bool need_bias) {
  ...
  if (do_trace) {
    ...
    if (trace_fp != NULL) {
      fprintf(trace_fp, "\n=== FORWARD PASS (pos=%d, token=%d) ===\n", pos,
              token);
      fprintf(trace_fp, "Input token embedding shape: [%d]\n",
  ...
  for (unsigned long long l = 0; l < p->n_layers; l++) {
    ...
    if (do_trace && trace_fp != NULL) {
      fprintf(trace_fp, "\n--- Layer %llu ---\n", l);
      fprintf(trace_fp, "1. Input RMSNorm: [%d] -> [%d]\n", dim, dim);
      fprintf(trace_fp, "2. Q_proj: input [%d] * weight [%d, %d] -> q [%d]\n",
              dim, dim, dim, dim);
      fprintf(trace_fp, "3. K_proj: input [%d] * weight [%d, %d] -> k [%d]\n",
              dim, dim, kv_dim, kv_dim);
      fprintf(trace_fp, "4. V_proj: input [%d] * weight [%d, %d] -> v [%d]\n",
              dim, dim, kv_dim, kv_dim);
      if (need_bias)
        fprintf(trace_fp, "5. Add Bias to Q, K, V\n");
      fprintf(trace_fp, "6. Apply RoPE to Q, K\n");
      fprintf(
          trace_fp,
          "7. ============= FlashAttention Module Equivalent =============\n");
      fprintf(
          trace_fp,
          "   Inputs: Q [%d, %d], K_cache [%d, %d, %d], V_cache [%d, %d, %d]\n",
          p->n_heads, head_size, pos + 1, p->n_kv_heads, head_size, pos + 1,
    ...
    // multihead attention using FlashAttention C-Model (Q8.8 behavior)
    t0 = get_time_sec();
    ...
    if (do_trace && trace_fp != NULL) {
      ...
      fprintf(trace_fp, "     - QK^T Dot Products: %d times ([%d] dot [%d])\n",
              pos + 1, head_size, head_size);
      fprintf(trace_fp, "     - Softmax: over %d elements\n", pos + 1);
      fprintf(trace_fp, "     - P*V Weighted Sums: %d times (scalar * [%d])\n",
              pos + 1, head_size);
      fprintf(trace_fp,
              "   Output: Attention_Out [%d, %d] -> Flattened to [%d]\n",
              p->n_heads, head_size, dim);
      fprintf(
          trace_fp,
          "   ==========================================================\n");
    }
    ...
    if (do_trace && trace_fp != NULL) {
      fprintf(trace_fp, "8. O_proj: input [%d] * weight [%d, %d] -> o [%d]\n",
              dim, dim, dim, dim);
      fprintf(trace_fp, "9. Add Residual (Attention Out + Input)\n");
      fprintf(trace_fp, "10. Post-Attention RMSNorm: [%d] -> [%d]\n", dim, dim);
      fprintf(
          trace_fp,
          "11. FFN Up/Gate Proj: input [%d] * W1/W3 [%d, %d] -> hb/hb2 [%d]\n",
          dim, dim, hidden_dim, hidden_dim);
      fprintf(trace_fp, "12. FFN SwiGLU Activation\n");
      fprintf(trace_fp,
              "13. FFN Down Proj: input [%d] * W2 [%d, %d] -> xb [%d]\n",
              hidden_dim, hidden_dim, dim, dim);
      fprintf(trace_fp, "14. Add Residual (FFN Out + Input)\n");
    }
...
void chat(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler,
  ...
  while (pos < steps) {
    ...
    if (pos == 0) {
      ...
      if (tf) {
        ...
        fprintf(tf,
                "                     PREFILL PHASE                        \n");
        fprintf(tf,
                "             (First Token Generation Trace)               \n");
        fprintf(tf,
                "==========================================================\n");
        fclose(tf);
      }
    } else if (pos == prompt_token_num) {
      do_trace = 1;
    ...
    if (pos > steps || (pos >= prompt_token_num && next == 151645)) {
      if (next == 151645)
        printf("\n");
      // ensure we capture decode time safely
      if (start != 0) {
        prefill = prefill == 0 ? time_in_ms() - start : prefill;
      }
      break;
  ...
  if (pos > 1 || flash_attention_sigint_requested()) {
    ...
    *decode_throughput = decode_token_count / (double)(dec_time) * 1000;
    ...
    printf("Total MACs(G): QKV=%8.3f, ATTN=%8.3f, O=%8.3f, FFN=%8.3f\n",
           total_macs_qkv / 1e9, total_macs_attn / 1e9, total_macs_o / 1e9,
           total_macs_ffn / 1e9);
    printf("Total Time(s): QKV=%8.3f, ATTN=%8.3f, O=%8.3f, FFN=%8.3f\n",
           time_qkv, time_attn, time_o, time_ffn);
    printf("Speed(GMAC/s): QKV=%8.2f, ATTN=%8.2f, O=%8.2f, FFN=%8.2f\n",
           (total_macs_qkv / 1e9) / time_qkv,
           (total_macs_attn / 1e9) / time_attn, (total_macs_o / 1e9) / time_o,
           (total_macs_ffn / 1e9) / time_ffn);

    double time_all = time_qkv + time_attn + time_o + time_ffn;
    long long total_macs =
        total_macs_qkv + total_macs_attn + total_macs_o + total_macs_ffn;
    printf("\n>>> Pipeline Tput: %.2f GMAC/s (%.2f GOPS)\n",
           (total_macs / 1e9) / time_all, 2 * (total_macs / 1e9) / time_all);
    printf(">>> ATTN time-%%  : %.1f %%\n", 100.0 * time_attn / time_all);
    printf("-----------------------------------------\n");
    printf("         OPERATION COUNTS\n");
    printf("-----------------------------------------\n");
    printf("Matmuls      : %lld calls\n", count_matmul);
    printf("RMSNorms     : %lld calls\n", count_rmsnorm);
    printf("Softmax      : %lld calls\n", count_softmax);
    printf("Attn QK Dots : %lld vectors\n", count_attn_dot);
    ...
    printf("Softmax      : %.1f\n", (float)count_softmax / total_tokens);
    printf("Attn Ops/Hd  : %.1f (dependent on pos length)\n",
           (float)count_attn_dot / total_tokens / transformer->config.n_heads);
    printf("=========================================\n\n");
  }
