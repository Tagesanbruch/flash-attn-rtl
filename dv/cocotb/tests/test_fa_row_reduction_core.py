...
async def drive_row(dut, scores, values, wait_cycles=40):
...
async def test_row_reduction_simple(dut):
    ...
    assert abs(got - ref) <= 3, f"simple row mismatch: got={got}, ref={ref}"
...
async def test_row_reduction_multi_rows(dut):
    ...
    for row_idx in range(5):
        ...
        ref = row_reduction_ref(scores, values)
        got = await drive_row(dut, scores, values, wait_cycles=50)
        assert got is not None, f"Row {row_idx}: No o_row_out_valid"
        assert abs(got - ref) <= 3, f"Row {row_idx}: got={got}, ref={ref}"
        await ClockCycles(dut.clk, 5)
...
async def test_row_reduction_edge_cases(dut):
    ...
    assert abs(got - ref) <= 3, f"Edge case 1: got={got}, ref={ref}"

    await ClockCycles(dut.clk, 5)

    scores = [-2048, -1024, -512, -256]
    values = [-512, -256, 256, 512]
    ref = row_reduction_ref(scores, values)
    got = await drive_row(dut, scores, values)
    assert got is not None, "Edge case 2: No valid"
    assert abs(got - ref) <= 3, f"Edge case 2: got={got}, ref={ref}"
