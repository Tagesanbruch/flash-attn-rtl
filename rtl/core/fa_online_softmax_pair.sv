module fa_online_softmax_pair #(
  ...
  input  logic signed [63:0]       i_acc_old0 [D],
  input  logic signed [63:0]       i_acc_old1 [D],
  input  logic signed [15:0]       i_v_row [D],
  ...
  output logic signed [63:0]       o_acc_new0 [D],
  output logic signed [63:0]       o_acc_new1 [D]
);
  ...
  always_comb begin
    ...
    for (int k = 0; k < D; k++) begin
      ...
      pv_mul = $signed({1'b0, o_exp_new0}) * i_v_row[k];
      pv_term = {{30{pv_mul[33]}}, pv_mul[33:0]} <<< 1;
      o_acc_new0[k] = acc_old_sc + pv_term;

      if (i_row1_valid) begin
        acc_sc = i_acc_old1[k] * $signed({1'b0, o_exp_old1});
        acc_old_sc = acc_sc[78:15];
        pv_mul = $signed({1'b0, o_exp_new1}) * i_v_row[k];
        pv_term = {{30{pv_mul[33]}}, pv_mul[33:0]} <<< 1;
        o_acc_new1[k] = acc_old_sc + pv_term;
      end else begin
        o_acc_new1[k] = i_acc_old1[k];
      end
    end
  end
endmodule