module fa_online_softmax_ctx (
  ...
  logic               s4_valid, s4_row_end;
  logic [1:0]         s4_ctx_id;
  logic signed [15:0] s4_m_new;
  logic [31:0]        s4_l_new;
  logic signed [31:0] s4_acc_new;
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    ...
    if (!rst_n) begin
      ...
      if (i_valid) begin
        ...
        if (s4_valid && (s4_ctx_id == i_ctx_id)) begin
          fwd_m_prev = s4_m_new;
          fwd_l_prev = s4_l_new;
          fwd_acc_prev = s4_acc_new;
        end else if (s3_valid && (s3_ctx_id == i_ctx_id)) begin
          fwd_m_prev = s3_m_new;
      ...
      if (s3_valid) begin
        s4_valid <= 1'b1;
        s4_row_end <= s3_row_end;
        s4_ctx_id <= s3_ctx_id;
        s4_m_new <= s3_m_new;
        s4_l_new <= s3_l_scaled_q32_31[46:15] + {15'd0, s3_exp_new, 1'b0};
        s4_acc_new <= s3_acc_scaled_q33_31[46:15] + {{5{s3_v_mul_q9_23[33]}}, s3_v_mul_q9_23[33:7]};
      end else begin
        s4_valid <= 1'b0;
      end

      o_valid <= s4_valid;
      o_row_done <= s4_valid && s4_row_end;
      o_ctx_id <= s4_ctx_id;
      o_m_q8_8 <= s4_m_new;
      if (s4_valid) begin
        idx = s4_ctx_id;
        m_state[idx] <= s4_m_new;
        l_state[idx] <= s4_l_new;
        acc_state[idx] <= s4_acc_new;
        o_l_q16_16 <= s4_l_new;
        o_acc_q16_16 <= s4_acc_new;
      end
    end
  end
endmodule
