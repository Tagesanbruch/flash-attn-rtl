...
for i, prompt in enumerate(prompts, 1):
    dbg = log_dir / f'sw_chat_prompt{i}_decode16.log'
    env = dict(os.environ)
    env.update({
        'RUN_FA_DEBUG_TOPK': '5',
    ...
    out = proc.stdout
    (log_dir / f'sw_chat_prompt{i}.stdout.log').write_text(out)
...
report = log_dir / 'sw_prompt_regression_chat_summary.txt'
