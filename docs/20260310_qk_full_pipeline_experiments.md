...
| fa_qk_dotprod_slice_pipe | exp_g | 272855.24 | -0.650 | 377.3 | -509.414 | `mult1_r[23]_31__reg_p:D` |
| fa_qk_dotprod_slice_pipe | exp_h | 286906.48 | -0.260 | 442.5 | -66.142 | `mult0_r[24]_31__reg_p:D` |

说明：

- 重新以前台方式直接调用 iEDA 包装脚本并在终端观察输出后，`exp_g` / `exp_h` 的 STA 都能在十几秒内完成；
- 之前 `sta.log` 缺失并不是设计不可分析，而是 Docker wrapper 调用阶段曾出现连接异常或被中途打断；
- 从 `exp_f` 到 `exp_h` 的结果已经足够验证主假设：**把首级改成纯乘法后，QK 频率可从 ~`203.8MHz` 拉升到 `381.9MHz~442.5MHz` 区间。**
...
| `exp_g` | `exp_f` + 位宽收敛 | 272855.24 | 377.3 | 面积略降，但频率略低于 `exp_f` |
| `exp_h` | `exp_g` + 输入预寄存 | 286906.48 | 442.5 | 额外寄存换来显著频率收益 |

结论非常明确：

- `exp_e -> exp_f` 频率提升约 `+178MHz`；
- `exp_e -> exp_h` 频率提升约 `+238.7MHz`；
- `exp_g` 说明单独做位宽收敛并不会自动继续提频，真正有效的是进一步缓解输入到乘法器阵列的扇出压力；
- 当前关键路径已经不再是加法树，而是单个乘法器输出到 `mult*_r`。
...
补跑 STA 后可以更明确地看出两者差别：

- `exp_g` 的位宽收敛主要带来了轻微面积下降，但对频率并未形成继续提升，说明当前主瓶颈已经不是高位冗余加法器，而是输入驱动乘法器阵列；
- `exp_h` 在 `exp_g` 基础上增加输入预寄存后，最差 slack 直接改善到 `-0.260ns`，估算频率达到 `442.5MHz`，这说明“输入大总线打一拍”对缓解乘法器前级扇出压力是有效的。

因此，若后续要把这一类结构集成回主线，优先级已经变得很清楚：

- `exp_f` 提供了“mult-only + fully-pipelined tree”的最小有效模板；
- `exp_h` 则说明在不改变 `32-lane` 与 `II=1` 的前提下，再加一拍输入寄存仍有显著收益。
...
1. 主线 QK 首先应参考 `exp_f/exp_h` 这一路线，而不是继续沿 `exp_e` 的局部乘加块细修；
2. 若目标是尽量逼近 `500MHz`，优先评估把 `exp_h` 风格的输入预寄存与流式 tag 控制集成回 `fa_attention_core`；
3. 若目标更偏面积平衡，则可保留 `exp_f` 作为结构上更简洁的过渡版本；
